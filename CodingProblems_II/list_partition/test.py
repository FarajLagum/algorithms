def list_partation(lst, x):
    ...
    while mid <= high:
        # If the current element is less than x, swap it with the low element and increment both low and mid
        if lst[mid] < x:
        ...
        # If the current element is equal to x, just increment mid
            ...
            mid += 1
        # If the current element is greater than x, swap it with the high element and decrement high
        else:
